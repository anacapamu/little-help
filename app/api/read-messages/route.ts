...
async function handler(req: NextRequest) {
    ...
    try {
        let messagesQuery;
        if (conversationId) {
            messagesQuery = query(collection(db, 'messages'), where('conversationId', '==', conversationId), orderBy('timestamp'));
        } else if (currentUserId) {
                const senderQuery = query(collection(db, 'messages'), where('sender.id', '==', currentUserId), orderBy('timestamp', 'desc'));

                const receiverQuery = query(collection(db, 'messages'), where('receiver.id', '==', currentUserId), orderBy('timestamp', 'desc'));
                ...
                const combinedMessages = [...senderMessages.docs, ...receiverMessages.docs]
        ...
        const messages = querySnapshot.docs.map(doc => ({ ...(doc.data() as MessageType)}));
